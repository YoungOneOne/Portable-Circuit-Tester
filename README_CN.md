...
│   └── PCB制板文件.zip                  # Gerber 生产文件，可直接用于板厂打样
...
*   **电路黑箱测试 (Black Box Mode)**：
    ...
    *   **参数计算**：高精度解算黑箱中的固定电阻 $R$（$10\Omega \sim 10\text{k}\Omega$）以及未知元件 $X$ 的具体数值（电阻 $Rx$、电容 $Cx$ 或电感 $Lx$），测量误差均远低于课设指标要求。
...
系统提取与过渡带中点增益 $Au_{mid} = (Au_{min} + Au_{max})/2$ 最接近的实测点频点 $f_x$，带入阻抗分压公式直接求出 $C$ 或 $L$。该算法相比于传统的捕捉 $-3\text{dB}$ 截止频率点的方法，极大地降低了对频点采样密度的依赖，测量误差从 $>20\%$ 降低到了 $\le 5\%$。
...
*   **解决高阻值测试瞬态建立迟滞（硬件时间常数优化）**：
    在大电阻测试时，由于交流隔离电容充放电时间常数 $\tau = R_x \cdot C$ 偏大导致电压建立极其缓慢（需约 5 秒），造成单片机采样读数误差显著增大。项目通过将该耦合电容**优化为 100nF (104独石电容)**，使时间常数降至 **$10\text{ms}$ 级**，过渡过程缩短至 $50\text{ms}$ 内，在保证交流阻隔离特性的同时实现了直流采样的快速稳态建立。
*   **DDS 高频幅值补偿**：
    DDS 发生器高频输出幅度存在自然衰减，系统代码在扫频循环中进行了分段幅度补偿（`AD9833_AmpSet`），保证了全频带内测量激励幅度的基本恒定。
*   **低功耗控制引脚电压不匹配软件兼容**：
    硬件上低功耗关机芯片的关机控制引脚需要达到 $3.5\text{V}$ 的高电平触发阈值，而 STM32 的 GPIO 输出最高为 $3.3\text{V}$ 导致无法触发自动关机。项目通过在软件上拉低与按键端相连的 PA0 引脚 3 秒，利用模拟物理长按按键的手法成功绕开电平限制，实现了系统待机电流 $\le 0.1\text{mA}$。
...
2.  在 Keil 中点击 **Rebuild**（重新构建项目，一键直接编译通过）。
...
## 🤝 贡献与致谢
*   **硬件开发与结构设计**：学生B (YYYYYYYY@example_univ.edu.cn)
*   **嵌入式固件与算法逻辑**：学生A (XXXXXXXX@example_univ.edu.cn)
*   **系统调测与文档分析**：学生C (ZZZZZZZZ@example_univ.edu.cn)
*   **指导教师**：教师A 老师
